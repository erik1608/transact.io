...
public class JWTPreAuthenticationFilter extends OncePerRequestFilter {
	...
	private static final String USER_INFO_PATH = "/api/*/user/**";
	private static final String BIOMETRY_REG_PATH = "/api/*/biometry/reg**";
	private static final String USER_ASSOCIATION_PATH = "/api/*/user/association**";

	private static final RequestMatcher USER_INFO_AUTHORIZED_REQUEST_GET_MATCHER = new AntPathRequestMatcher(USER_INFO_PATH, HttpMethod.GET.name());
	private static final RequestMatcher USER_INFO_AUTHORIZED_REQUEST_POST_MATCHER = new AntPathRequestMatcher(USER_INFO_PATH, HttpMethod.POST.name());
	private static final RequestMatcher BIOMETRY_AUTHORIZED_REQUEST_POST_MATCHER = new AntPathRequestMatcher(BIOMETRY_REG_PATH, HttpMethod.POST.name());
	private static final RequestMatcher USER_ASSOCIATION_REQUEST_POST_MATCHER = new AntPathRequestMatcher(USER_ASSOCIATION_PATH, HttpMethod.POST.name());
	private static final RequestMatcher USER_ASSOCIATION_REQUEST_GET_MATCHER = new AntPathRequestMatcher(USER_ASSOCIATION_PATH, HttpMethod.GET.name());
	...
	protected void doFilterInternal(@NonNull HttpServletRequest request,
		...
		JsonObject requestJson = new Gson().fromJson(new InputStreamReader(requestWrapper.getInputStream()), JsonObject.class);
		...
		try {
			Session sessionData = new Gson().fromJson(requestJson.get(SESSION_DATA_KEY), Session.class);
	...
	private void unauthorized(HttpServletResponse response) throws IOException {
		...
		responseJson.addProperty("message", "A session is required for this endpoint");
