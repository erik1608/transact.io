...
public class UserFriendRequest implements RequestModel {
	...
	public void validate() {
		...
		if (sessionData == null) {
			throw new RequestValidationException("Session is required for this endpoint");
		}
	...
	private void validateGetRequest() {
		if (status == null || status.isEmpty()) {
			throw new RequestValidationException("status is required");
		}

		if (!FRIEND_REQUEST_STATUSES.contains(status)) {
			throw new RequestValidationException("Unknown status provided");
		}
	}

	private void validateUpdateRequest() {
		if (action == null || action.isEmpty()) {
			throw new RequestValidationException("action is required");
		}
